...
def dotgraphTree(decisionTree):
    dcNodes = defaultdict(list)
    """Plots the obtained decision tree. """
    def toString(iSplit, decisionTree, bBranch, szParent = "null", indent=''):
        if decisionTree.is_leaf:  # leaf node
            dcNodes[iSplit].append(['leaf', str(decisionTree.index), szParent, bBranch, decisionTree.impurity,
                                    len(decisionTree.data_idx)])
            return
        else:
            szCol = 'Column %s' % decisionTree.split_index
            decision = '%s >= %s' % (szCol, decisionTree.split_value)
            trueBranch = toString(iSplit+1, decisionTree.left_child, True, decision, indent + '\t\t')
            falseBranch = toString(iSplit+1, decisionTree.right_child, False, decision, indent + '\t\t')
            
            dcNodes[iSplit].append([iSplit+1, decision, szParent, bBranch, decisionTree.impurity,
                                    len(decisionTree.data_idx)])
            return
